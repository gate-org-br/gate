 ...
 * compatible when every provided attribute matches one of its parameters.
 * <p>
 * When multiple compatible constructors or factories are available, the selection is strict:
 * <ul>
 *     <li>a candidate is selected automatically only when there is a single exact match, that is,
 *     a single compatible executable whose number of parameters is equal to the number of provided
 *     attributes;</li>
 *     <li>if both a constructor and a factory remain viable, or if more than one exact match
 *     exists, the resolution is considered ambiguous and fails;</li>
 *     <li>compatible executables with extra parameters do not win over an exact match.</li>
 * </ul>
 * <p>
...
public interface ConstructionStrategy
{
	...
	static ConstructionStrategy get(Class<?> type, Set<Attribute> attributes)
	{
		return CACHE.computeIfAbsent(attributes, e ->
		{
			try
			{
				...
				{
					constructors = constructors.stream().filter(c -> c.getParameterCount() == attributes.size()).toList();
					factories = factories.stream().filter(c -> c.getParameterCount() == attributes.size()).toList();

					if (constructors.size() == 1 && factories.isEmpty())
						return new ConstructorStrategy(constructors.get(0));
					if (constructors.isEmpty() && factories.size() == 1)
	...
	private static List<Attribute> getConstructorAttributes(Executable executable,
