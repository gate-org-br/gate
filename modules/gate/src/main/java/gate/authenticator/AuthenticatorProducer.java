		...
		return authenticators
				...
				.computeIfAbsent(index, key ->
				{
					...
					if (config.getProperty("type").isPresent())
						...
						{
							case "database" -> new DatabaseAuthenticator(userCatalog);
							case "ldap" -> new LDAPAuthenticator(config, userCatalog);
							case "oidc" -> new OIDCAuthenticator(config, userCatalog);
					...
					return new DatabaseAuthenticator(userCatalog);
				});
	}
