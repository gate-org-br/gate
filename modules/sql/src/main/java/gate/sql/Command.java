...
public class Command implements AutoCloseable, Fetchable, ColumnWriter
{
	...
	@Override
	public Command onClose(Runnable onClose)
	{
	...
	public short getGeneratedByteKey()
	{
		try (Cursor cursor = getGeneratedKeys())
		{
	...
	public int getGeneratedLongKey()
	{
		try (Cursor cursor = getGeneratedKeys())
		{
			return cursor.next() ? cursor.getCurrentIntValue() : 0;
	...
	public void close()
	{
		try
		{
			...
			for (Runnable onClose : onCloses)
				try {onClose.run();} catch (Exception ignore) {}
		} catch (SQLException ex)
		{
	...
	public int setParameter(Class<?> type, int index, Object object)
	{
		try
		{
			return ColumnMapper.getColumnMapper(type)
					.writeToPreparedStatement(this, index, object);
		} catch (SQLException ex)
		{
