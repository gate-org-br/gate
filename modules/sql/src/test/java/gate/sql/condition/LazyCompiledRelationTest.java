...
public class LazyCompiledRelationTest
{
    ...
    @Test
    public void shouldPreserveConnectorsWhenLazyRelationIsEnabled()
    {
    ...
    @Test
    public void shouldSkipLazySuppliersWhenRelationIsDisabled()
    {
    ...
    @Test
    public void shouldSkipPropertyReferenceResolutionWhenLazyRelationIsDisabled()
    {
    ...
    @Test
    public void shouldBuildCompiledConditionsAndExistsFromSuppliers()
    {
        Condition condition = Condition.of("id").eq(1)
                .and().when(true).condition((LazyCompiledRelation.CompiledConditionSupplier) () ->
                        Condition.of("name").eq("User 1"))
        ...
        assertEquals("id = ? and (name = ?) and exists (select sub_id from sub_table where active = ?)",
                condition.toString());
        assertEquals(List.of(1, "User 1", 2), condition.getParameters().toList());
    }
}
