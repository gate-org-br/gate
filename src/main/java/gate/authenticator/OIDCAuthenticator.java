		...
		try {
			...
			if (tokens.containsKey("id_token")) {
				Jwt<Header, Claims> jwt = tokens.getString("id_token")
						...
						.orElseThrow(
								() -> new AuthenticatorException("Error trying to get id token from auth provider"));
			...
			String accessToken = tokens
					...
					.orElseThrow(
							() -> new AuthenticatorException("Error trying to get access token from auth provider"));
			...
			return control.select(userInfo.getString(userId.get())
					.orElseThrow(
							() -> new AuthenticatorException("Error trying to get user user id from auth provider")));
