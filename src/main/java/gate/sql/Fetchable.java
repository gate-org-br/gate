...
/**
 * Object that can fetch query results in different formats.
 */
public interface Fetchable
{

	/**
	 * Fetches the current result set using the specified fetcher.
	 *
	 * @param fetcher fetcher used to consume the result set
	 * @return fetched result
	 */
	<T> T fetch(Fetcher<T> fetcher);

	/**
	 * Streams the current result set using the specified row mapper.
	 *
	 * @param mapper mapper used to convert each row
	 * @return stream of mapped rows
	 */
	...
	/**
	 * Fetches the first column of each row as a list of Java objects.
	 *
	 * @return the first column of each row as a list of Java objects
	 */
	...
	/**
	 * Fetches the first column of each row as a set of Java objects.
	 *
	 * @return the first column of each row as a set of Java objects
	 */
	...
	/**
	 * Fetches the first column of each row as a list of Java objects of the specified type.
	 *
	 * @param type type of the objects to be fetched
	 * @return the first column of each row as a list of Java objects of the specified type
	 */
	...
	/**
	 * Fetches the first column of each row as a set of Java objects of the specified type.
	 *
	 * @param type type of the objects to be fetched
	 * @return the first column of each row as a set of Java objects of the specified type
	 */
	...
	/**
	 * Fetches the current result set as a {@link Page} of maps whose keys are the column names and
	 * values are the column values.
	 *
	 * <p>The query must include a {@code dataSize} column containing the total number of rows
	 * that match the filter, ignoring pagination. The usual way to provide it is with a window
	 * function such as {@code count(*) over() as dataSize}.</p>
	 *
	 * <p>This method does not issue a separate {@code count(*)} query. It only consumes the
	 ...
	 * @throws UnsupportedOperationException if the result set does not contain a {@code dataSize} column
	 */
	default Page<Map<String, Object>> fetchMapPage(int pageSize, int pageIndx)
	{
		return fetch(new MapPageFetcher(pageSize, pageIndx));
	}

	/**
	 * Fetches the current result set as a {@link Page} of typed maps.
	 *
	 * <p>The query must include a {@code dataSize} column containing the total number of rows
	 * that match the filter, ignoring pagination. The usual way to provide it is with a window
	 * function such as {@code count(*) over() as dataSize}.</p>
	 *
	 * <p>This method does not issue a separate {@code count(*)} query. It only consumes the
	 * current result set and maps each non-{@code dataSize} column using the corresponding entry
	 * from {@code types}.</p>
	 *
	 * @param pageSize number of rows per page
	 * @param pageIndx index of the page
	 * @param types    types of the objects to be fetched for each non-{@code dataSize} column
	 * @return a page containing the current rows and the total size informed by {@code dataSize}
	 * @throws UnsupportedOperationException if the result set does not contain a {@code dataSize} column
	 */
	...
	/**
	 * Fetches the first row as a Java object of the specified type, allowing contextual values
	 ...
	 * @return an Optional describing the first row of the result as a Java object of the specified type
	 */
	default <T> Optional<T> fetchEntity(Class<T> type, Function<String, Object> context)
	{
		return fetch(new EntityFetcher<>(type, context));
	}

	/**
	 * Fetches each row as a list of Java objects of the specified type with its properties set to their respective column values.
	 *
	 * @param type type of the objects to be fetched
	 * @return each row as a list of Java objects of the specified type with its properties set to their respective column values
	 */
	...
	/**
	 * Fetches each row as a list of Java objects of the specified type, allowing contextual values
	 * to be supplied for specific properties before they are read from the result set.
	 *
	 * <p>The {@code context} may provide a value for a property, identified by its full name,
	 * before the cursor tries to read it from the current row. Returning {@code null} means
	 ...
	 * @param type    type of the objects to be fetched
	 * @param context function used to provide contextual values for specific property names
	 * @return each row as a list of Java objects of the specified type with their properties set to their respective column values
	 */
	...
	/**
	 * Fetches the current result set as a {@link Page} of Java objects of the specified type.
	 *
	 * <p>The query must include a {@code dataSize} column containing the total number of rows
	 * that match the filter, ignoring pagination. The usual way to provide it is with a window
	 * function such as {@code count(*) over() as dataSize}.</p>
	 *
	 * <p>This method does not issue a separate {@code count(*)} query. It only consumes the
	 * current result set and materializes each row using the same entity construction rules used
	 * by the rest of the framework.</p>
	 *
	 * @param type     type of the objects to be fetched
	...
	/**
	 * Fetches the current result set as a {@link Page} of Java objects of the specified type,
	 * allowing contextual values to be supplied for specific properties before they are read
	 * from the result set.
	 *
	 * <p>The query must include a {@code dataSize} column containing the total number of rows
	 * that match the filter, ignoring pagination. The usual way to provide it is with a window
	 ...
	 * @throws UnsupportedOperationException if the result set does not contain a {@code dataSize} column
	 */
	default <T> Page<T> fetchEntityPage(Class<T> type, int pageSize, int pageIndx, Function<String, Object> context)
	{
		return fetch(new EntityPageFetcher<>(type, pageSize, pageIndx, context));
	}

	/**
	 * Fetches each row as a set of Java objects of the specified type with its properties set to their respective column values.
	 *
	 * @param type type of the objects to be fetched
	 * @return each row as a set of Java objects of the specified type with its properties set to their respective column values
	 */
