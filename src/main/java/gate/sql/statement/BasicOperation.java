		...
		public class Connected implements Operation.Compiled.Connected<T>
		{
			...
			public int execute() throws ConstraintViolationException
			{
				try (Command command = link.createCommand(sql))
				{
					...
					{
						Property entityId = Property.getProperty(type, EntityHelper.getId(type));
				...
				public int execute() throws ConstraintViolationException
				{
					try (Command command = link.createCommand(sql))
					{
						...
						if (EntityHelper.isEntity(type))
						{
							...
							for (T value : values)
							{
								properties.forEach(e -> command.setParameter(e.getRawType(),
										e.getValue(value)));
								result += command.execute();
								if (entityId.getValue(value) == null)
									command.getGeneratedKey(entityId.getRawType())
					...
					if (EntityHelper.isEntity(type))
					{
						...
						for (T value : values)
						{
							properties.forEach(
									e -> command.setParameter(e.getRawType(), e.getValue(value)));
							result += command.execute();
							var type = entityId.getRawType();
							if (type == int.class)
								entityId.setInt(value, command.getGeneratedIntKey());
							else if (type == long.class)
								entityId.setInt(value, command.getGeneratedLongKey());
							else
								command.getGeneratedKey(type)
										.ifPresent(id ->
												entityId.setValue(value, id));
						}
					} else
					{
						for (T value : values)
