...
public class UpdateLinkTag extends AnchorTag
{
	...
	public void doTag() throws JspException, IOException
	{
		...
		Method method = Screen.getAction(type, "Update").orElseThrow(() -> new JspException("Requisição inválida"));

		if (Gate.checkAccess(user, module, screen, "Update", type, method))
		{
			...
			getAttributes().putIfAbsent("title", Description.Extractor.extract(method)
				.or(() -> Name.Extractor.extract(method)).orElse("Alterar"));
			getAttributes().put("href", "Gate?" + parameters.toString());

			getJspContext().getOut().println("<a " + getAttributes() + ">");
			if (getJspBody() == null)
				getJspContext().getOut().println(Name.Extractor.extract(method).orElse("Alterar") + "<i>"
					+ Icon.Extractor.extract(method).or(() -> Icons.getInstance().get("update")).orElse(Icons.UNKNOWN) + "</i>");
			else
