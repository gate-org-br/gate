...
public class SelectAttributeProcessor extends FormControlAttributeProcessor
{
						...
						IElementTagStructureHandler handler, Screen screen, Property property, Object value)
	{
		...
		var children = extract(element, handler, "g:children").map(expression.create()::function).orElse(null);
		...
		if (groups != null)
		{
			Toolkit.stream(options)
					...
					.forEach((key, value1) ->
					{
						body.add("<optgroup label='" + Converter.toText(key) + "'>");
						print(0, body, value1, labels, values, children, value);
						body.add("</optgroup>");
					});
		} else
