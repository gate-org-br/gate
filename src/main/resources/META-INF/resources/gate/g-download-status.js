...
{
	let disposition = /^attachment;.*filename="([^"]*)"$/
		.exec(request.getResponseHeader('content-disposition'));
	if (disposition)
	...
	download(method, url, data)
	{
		...
		this.request.addEventListener("load", () =>
		{
			...
			if (this.request.status === 200)
			{
				...
				link.download = getFileName(this.request);
			...
			} else
			{
				...
				reader.addEventListener("loadend", function ()
				{
					...
					this.dispatchEvent(new CustomEvent('error', {'detail': reader.result}));
				});
