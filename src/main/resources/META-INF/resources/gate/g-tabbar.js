...
customElements.define("g-tabbar", class extends HTMLElement
{
	...
	connectedCallback()
	{
		...
		let selected = Array.from(this.children)
			.filter(e => window.location.href === (e.href || e.formaction))[0];
		if (!selected)
			selected = Array.from(this.children)
				.filter(e => window.location.href.startsWith(e.href || e.formaction))[0];
		if (selected)
			selected.setAttribute("aria-selected", "aria-selected");
	}
