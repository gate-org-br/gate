...
customElements.define('g-selectn', class extends HTMLElement
{
	...
	reportValidity()
	{
		...
		if (this.required && !checked)
			return false & Message.error("Selecione ao menos uma opção");

		if (this.min && checked < min)
			return false & Message.error(`Selecione ao menos ${min} opções`);

		if (this.max && checked > max)
			return false & Message.error(`Selecione no máximo ${max} opções`);
