...
function process (element)
{
	...
	if (element.value && datalist.hasAttribute("data-options"))
	{
		...
		} else if (!element.filter || !element.value.toLowerCase().includes(element.filter.toLowerCase()))
		{
			...
			fetch(resolve(datalist.getAttribute("data-options")))
...
DOM.forEveryElement(e => e.tagName === "INPUT" && e.hasAttribute("list"), element =>
{
	element.setAttribute("autocomplete", "off");
	element.addEventListener("input", () => process(element));
	process(element);
});